...
export function objectsCollectionOptions<TSchema extends StandardSchema<OntologyObject>>(
    ...
    const sync: SyncConfig<InferSchemaOutput<TSchema>, string | number> = {
        sync: (params) => {
            ...
            const safeDelete = (object: InferSchemaOutput<TSchema>) => {
                const existingObject = collection.get(object.__primaryKey);
                if (existingObject) {
                    console.log("Found existing object, deleting...", existingObject, object);
