...
describe("createOntologyBackendInstallation", () => {
    it("opens isolated LiveOntologies through one stable connection", async () => {
        const cleanupOntology =
            vi.fn();
        const destroyedNamespaces:
            string[] = [];
        ...
        const connection =
            await installation.authentication.signIn();
        ...
        expect(
            installation.connections.get(connection.userId)?.state.status
        ).toBe("inactive");
        await installation.forget(
            connection.userId
        );
        expect(
            installation.connections.get(
                connection.userId
            )
        ).toBeUndefined();
        expect(
            destroyedNamespaces.sort()
        ).toEqual([
    ...
    it("rejects ambiguous dynamic routes", async () => {
        const adapter =
            createTestConnectionAdapter();
        const installation =
            await createOntologyBackendInstallation({
                installationId:
                    "ambiguous-routes",
                connections: () =>
                    adapter,
        ...
        await expect(
            ...
            'Multiple ontology routes matched "matched"'
        );
        await installation.cleanup();
    });
    ...
    it("deduplicates concurrent opens and cleanup", async () => {
        ...
        await expect(
            ...
            'Backend installation "concurrent-open" is closed.'
        );
    });
});
