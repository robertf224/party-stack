...
describe("createClient", () => {
    ...
    it("treats a supplied fetch as already authenticated", async () => {
        ...
        const tokenProvider = vi.fn(() =>
            Promise.reject(
                new Error("Unexpected token access.")
            )
        );
        ...
        await client.fetch(
            "https://foundry.example.com/api"
        );

        expect(fetch).toHaveBeenCalledOnce();
        expect(tokenProvider).not.toHaveBeenCalled();
        expect(client.createWebSocket).toBe(
            createWebSocket
        );
    });
    ...
    it("uses tokenProvider when creating its default fetch", async () => {
        ...
        expect(
            (request as Request).headers.get(
                "authorization"
            )
        ).toBe("Bearer token");
    });
    ...
    it("creates origin-restricted authenticated WebSockets", async () => {
        ...
        await factory(
            "wss://foundry.example.com/stream",
            ["custom"]
        );
