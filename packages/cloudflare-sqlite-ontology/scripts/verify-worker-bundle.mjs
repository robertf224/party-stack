...
const result = await build({
    ...
    // Let tree-shaking discard unreachable platform modules, then assert below
    // that no Node external survived in the emitted Worker.
...
console.log("Worker bundle is free of better-sqlite3 and Node-only modules.");
