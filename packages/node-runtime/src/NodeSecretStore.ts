...
const ALLOWED_BACKEND_IDS = new Set([
    "native-macos",
    "macos",
    "native-linux",
    "secret-service",
...
async function selectOSKeychainBackend(): Promise<SecretStorageBackend> {
    const candidates = (await getAllBackends()).filter((backend) => ALLOWED_BACKEND_IDS.has(backend.id));
...
export class NodeSecretStore implements SecretStore {
    ...
    #backend?: Promise<SecretStorageBackend>;
    ...
    #getBackend(): Promise<SecretStorageBackend> {
        this.#backend ??= selectOSKeychainBackend();
        return this.#backend;
    }
}
