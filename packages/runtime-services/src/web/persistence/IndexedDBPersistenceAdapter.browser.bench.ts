...
const categoryEquals = queryOptions((query) =>
    query.where(({ item }) => eq(item.category, "category-42"))
);
const highPriority = queryOptions((query) =>
    query.where(({ item }) => gte(item.priority, 900))
);
const indexedAnd = queryOptions((query) =>
    query.where(({ item }) =>
        and(eq(item.category, "category-42"), gte(item.priority, 900))
    )
);
const indexedOr = queryOptions((query) =>
    query.where(({ item }) =>
        or(eq(item.category, "category-42"), gte(item.priority, 900))
    )
);
const residualAnd = queryOptions((query) =>
    query.where(({ item }) =>
        and(
            eq(item.category, "category-42"),
            not(eq(item.status, "open"))
        )
    )
);
...
const prefixLike = queryOptions((query) =>
    query.where(({ item }) => like(item.name, "prefix-%"))
);

const adapter = new IndexedDBPersistenceAdapter({
    databaseName: `runtime-services-browser-bench-${crypto.randomUUID()}`,
});
...
for (const [name, options, expected] of expectedCounts) {
    ...
    if (rows.length !== expected) {
        throw new Error(
            `${name} expected ${expected} candidate rows, received ${rows.length}.`
        );
    }
}
